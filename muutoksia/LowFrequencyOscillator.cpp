...
// buffer = sample buffer (output)
// firstSample = index of first sample in the buffer to calculate
// lastSample = index of last sample (inclusive)
void
LowFrequencyOscillator::generateSound(float buffer[], unsigned int firstSample,
unsigned int lastSample)
{
    ...
    float * ptr = buffer + firstSample;
    float * endPtr = buffer + lastSample + 1;

    while (ptr < endPtr) {
		// (int)(x + 0.5) is 3 times faster than (int)roundf()
    	// (Tested with GCC 4.6.3 and optimization -O2)

		*ptr++ = sineTable[(int)(angle * WAVE_TABLE_LENGTH + 0.5)];
		angle += increase;
		if (angle >= 1) {
			angle -= 1;
		}
