	...
	events(b), parameters(p), filter(p.lowpassFrequency)
{
	...
	for (i = 0; i < POLYPHONY; i++) {
		...
		lfo1[i] = new LowFrequencyOscillator(parameters.osc1, *oscillator1[i]);
	}
...
{
    synthInstance->processEvents(nframes);
...
void Synthesizer::generateSound(jack_nframes_t nframes)
{
	...
	if (useGlobalLfo) {
		lfo1[0]->generateSound(lfoBuffer, 0, bufferLength - 1);
	}
	...
	for (oscNum = 0; oscNum < POLYPHONY; oscNum++) {
		if (oscillator1[oscNum]->getEnvelopePhase() != OFF) {
			if (useDedicatedLfo) {
				lfo1[oscNum]->generateSound(lfoBuffer, 0, bufferLength - 1);
			}
			oscillator1[oscNum]->generateSound(oscillatorBuffer, lfoBuffer,
					0, bufferLength - 1, noteFinished);
			for (i = 0; i < nframes; i++) {
...
// Processes all types of events the Synthesizer can get from
// GUI and JACK.
void Synthesizer::processEvents(jack_nframes_t nframes)
{
	processGuiEvents();
	processMidiEvents(nframes);
	checkJackExceptions();
}
...
void Synthesizer::processGuiEvents()
{
	...
	while (ptr < endPtr) {
		...
		processNoteOn(key, velocity, 0, source);
		break;
...
{
	// First try to find and retrigger an oscillator group
    // with the same note key
	unsigned int i;
	for (i = 0; i < POLYPHONY; i++) {
		if (noteKey[i] == key && oscillator1[i]->getEnvelopePhase() != OFF)
		{
...
void Synthesizer::processNoteOff(unsigned char key, unsigned short time,
NoteSource source)
{
